...
type DropdownKeyMobile = 'personal' | 'business' | 'developer' | null;

function Navbar() {
  const [scrolled, setScrolled] = useState(false);
  const [showMobileMenu, setShowMobileMenu] = useState(false);
  const [openDropdown, setOpenDropdown] = useState<DropdownKey>(null);
  const [openMobileDropdown, setOpenMobileDropdown] = useState<DropdownKeyMobile>(null);
  ...
  return (
    <div className={`text-base fixed top-0 left-0 right-64 md:right-0 z-50 transition-all duration-300 ${
      ...
      <div className="mx-auto max-w-[1380px] px-5">
        <div className="flex items-center justify-between h-[75px]">
          <div className="flex items-center">
            <a href="#" className={`mt-[5px] mr-4 md:mr-14 ${showMobileMenu ? 'hidden' : 'block'} md:block`}>
            ...
            <NavbarDesktop openDropdown={openDropdown} toggleDropdown={toggleDropdown} />
          </div>

          <div className="hidden md:flex gap-4 items-center">
            <a href="#" className="text-[#0066f5] font-semibold">Log In</a>
            <a href="#" className="bg-[#0066f5] text-sm text-white px-4 py-3 rounded-md font-bold hover:shadow-2xl transition">Sign Up For Free</a>
          </div>
      ...
      <NavbarMobile show={showMobileMenu} setShow={setShowMobileMenu} openMobileDropdown={openMobileDropdown} setOpenMobileDropdown={setOpenMobileDropdown} isMobileMenuItemVisible={isMobileMenuItemVisible} />
    </div>
  );
}
